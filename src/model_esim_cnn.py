...
def main(argv):
    ...
    def build_model():
        ...
        if use_CuDNNLSTM:
            v_q1_i = CuDNNLSTM(LATENT_DIM, return_sequences=True)(m_q1)
            v_q2_i = CuDNNLSTM(LATENT_DIM, return_sequences=True)(m_q2)
        else:
        ...
        final_v = BatchNormalization()(concatenate([merged_q1, merged_q2, cnn_layer_q1, cnn_layer_q2]))
    ...
    def continue_train_model_kfold_k(k):
        """
        print_lines(train_path) #(719992, 11)
        print_lines(valid_path) #(180008, 11)
        print_lines(test_path) #(1000000, 9)
        ...
        for i, (train_index, valid_index) in enumerate(skf.split(data, data['age-gender'])):
            if (i+1) == k:
                ...
                # loss compile
                # optimizer = Adam(lr=1e-3, beta_1=0.9, beta_2=0.999, decay=0.0)
                adam_optimizer = Adam(lr=0.0005000000237487257, decay=5e-5, clipvalue=5)
                model.compile(optimizer=adam_optimizer,
                              loss='categorical_crossentropy', loss_weights=[0.9, 0.1],
                              metrics=['accuracy'])
    ...
    def pred_model_kfold_k(k):
        ...
        # user_id.to_csv(os.path.join(SUBMISSION_PATH, 'valid_%sfold%s.csv' % (VERSION, str(k+1))), index=False)
    ...
    elif MODE == "continue_train_kfold_k":
        print(MODE, FOLDK)
        continue_train_model_kfold_k(FOLDK)
